  ...
  enterFeatures.addEventListener("click", function (e) {
    ...
    for (let i = 0; i <= todosDOMArray.length; i++) {
      ...
      if (parseInt(idElement.textContent) === idNum) {
        ...
        let notesHeading = document.createElement("p");
        notesHeading.textContent = "Your notes:";
        descriptionHeading.classList.add("heading");
        dueDateHeading.classList.add("heading");
        notesHeading.classList.add("heading");
        idElement.parentNode.insertBefore(
          featuresDiv,
          idElement.parentNode.children[2]
        );
        featuresDiv.appendChild(headingsDiv);
        featuresDiv.appendChild(infoDiv);
        headingsDiv.appendChild(descriptionHeading);
        headingsDiv.appendChild(dueDateHeading);
        headingsDiv.appendChild(notesHeading);
        infoDiv.appendChild(description);
        infoDiv.appendChild(dueDate);
        infoDiv.appendChild(notes);
        addFeaturesArray[idNum].textContent = "Edit";
        addFeaturesArray[idNum].classList.add("edit");
        editFeaturesEvent();
...
let addFeaturesEvent = () => {
  let addFeaturesForm = document.getElementById("addFeaturesForm");
  let featuresDOMButtonArray = [
    ...document.getElementsByClassName("addFeatures"),
  ]; //need for each
...
let editFeaturesEvent = () => {
  let editButtonsArray = [...document.getElementsByClassName("edit")];
  let editForm = document.getElementById("editForm");
  editButtonsArray.forEach(function (edit) {
